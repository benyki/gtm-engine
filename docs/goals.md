...
All four engine folders get scaffolded, but you don't configure all four. Do
...
- Paths are relative to your home (`~/gtm/`).
- **One video folder per format**, named after it: `video-app/` (viral product),
  ...
  `scaffold.py --merge --engine video-vibe:video`.
...
## 1. Viral app video engine — `video-app/`

Short, hook-led clips of the product. Volume format: the hook is the variable,
everything else is production.

- [ ] Engine folder exists, `type: video`, metric `watch_through_rate` — or `views` if you're not installing the browser extension, since `yt-dlp` reads public counters but never watch-through [file `video-app/engine.json` to fill]
...
- [ ] One video rendered, logged, published, URL recorded, and the mp4 moved to the archive [check: a row in `video-app/runs/index.csv` with a `url`, and the file in that engine's `published_dir`]
...
## 2. Viral vibe video engine — `video-vibe/`
...
- [ ] Engine folder exists, `type: video`, its own goal and metric [file `video-vibe/engine.json` to fill]
...
- [ ] The overlay style, unchanged from the other video engines unless the look *is* the point [file `video-vibe/templates/floating-text-default.json` to fill]
...
- [ ] One video rendered, logged, published, URL recorded, mp4 in that engine's `published_dir` [check]
- [ ] One format you'd ship unedited, experiment paused [file `video-vibe/experiments.json` to fill — leave `status: paused`]

---

## 3. Informative video engine — `video-info/`

Same machinery, different job: turn text you didn't write into a watchable
explainer.

- [ ] Engine folder exists, `type: video`, its own goal and metric [file `video-info/engine.json` to fill]
...
- [ ] Engine folder, goal, metric (clicks from Search Console) [file `seo/engine.json` to fill]
...
- [ ] ≥3 markdown articles you like, in the engine, as the voice reference [folder `seo/inputs/best/` to create]
...
- [ ] Engine folder, one platform chosen for today [file `social/engine.json` to fill]
- [ ] Subject flow set up for this engine — `engine-social/references/subject-finding.md`, validated on the platform rather than on search volume [file `social/sources.json` to fill]
- [ ] ≥20 validated rows in this engine's own backlog, each with a claim, its proof and a score [file `social/inputs/backlog.csv` to fill]
...
- [ ] Engine folder, goal, metric = replies [file `outreach/engine.json` to fill]
...
- [ ] A lead list on disk **in any format you already have it** — the engine converts it, so don't reformat first — or an engine step that produces one; hand-build the first 20–50. B2B shortcut: LinkedIn search in the browser → a finder tool (Dux-Soup, Lusha, Apollo, Hunter) for the addresses → this engine [folder `outreach/inputs/audience/` to create] [skill `prospect-finder` to install]
...
- [ ] Daily job that reads the replies and records them — **any** reply is a 1, including a polite no; only a closed sequence with no reply at all is a 0. That *is* this engine's metric job; don't add a second one [daily scheduler `engine-metrics-outreach` to create]
...
## 7. The loop — true for every engine above

- [ ] **One metric job per engine you ran today**, on that channel's clock — daily for outreach replies and social, weekly or slower for Search Console [daily scheduler `engine-metrics-<engine>` to create — one each]
- [ ] Weekly job that renders the reports (and scores arms later, once anything is live) — **one for the whole home**, because reading the sibling reports together is the point [weekly scheduler `engine-weekly` to create]
- [ ] **No live experiments yet — that's correct.** Every engine ships one template and `experiments.json` stays paused until its format is settled and 5–10 pieces have numbers. `engine-loop/references/ab-testing.md` → R0 [check]
- [ ] Every engine has a `reports/latest.json` from a real run [check]
...
python3 ~/.gtm-engine/skills/engine-loop/scripts/due_metrics.py
python3 ~/.gtm-engine/skills/engine-loop/scripts/score_arms.py
...
One published thing per engine beats five folders of drafts. The schedulers
