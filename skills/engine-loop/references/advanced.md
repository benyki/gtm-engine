...
**Track time-to-outcome, not just outcome.** A reply in two hours and a reply in three weeks are different results. `created_at` and `replied_at` are already in the schema.
