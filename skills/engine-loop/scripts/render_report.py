#!/usr/bin/env python3
"""Write the weekly report — same six sections, every week.

Produces three things:

  reports/weekly-YYYY-Www.md   for you to read
  reports/latest.json          for the NEXT agent to read
  reports/index.csv            one row per report, ever

The JSON is the important one. An agent picking this workspace up next week
should read `reports/latest.json` before deciding anything — it gets the
...
def main() -> int:
    ...
    end, start = now, now - timedelta(days=a.days)
    prev_start = start - timedelta(days=a.days)
    ...
    if ch.is_file():
        try:
            metric_name = (json.loads(ch.read_text()).get("primary_metric") or "").strip()
    ...
    data = {
        ...
        "period": {"from": start.strftime("%Y-%m-%d"),
                   "to": end.strftime("%Y-%m-%d"), "days": a.days},
        "primary_metric": metric_name,
        "ran": {"total": len(this_runs), "by_workflow": counts(this_runs, "skill"),
    ...
    if this_runs:
        ...
        L += [""] + [f"- {k}: {v}" for k, v in data["ran"]["by_workflow"].items()]
    else:
    ...
    if metric_name:
        ...
        L.append("_No primary metric set in `config/channels.json`._")
    ...
    L += ["## 5. Proposed changes", "", NEEDS_HUMAN, "",
          "> A concrete diff to `config/`, or 'none'. If an experiment above is "
          "decided: promote the winner, move the loser to "
          "`templates/<workflow>/losers/`, write a challenger with its hypothesis.", "",
