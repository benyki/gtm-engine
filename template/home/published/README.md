...
**Everything that actually went out, in one place.** One subfolder per workflow,
created the first time that workflow ships something:
...
experiments, metrics — stays inside each workflow folder, so nothing here is
...
Set `published_dir` in a workflow's `workflow.json`. It's per workflow, so
different answers can coexist — video on an external drive, social in the
workspace:

| `published_dir` | Where artifacts go | Good when |
|---|---|---|
| *(empty — the default)* | `<workspace>/published/<workflow>/` | you want one place to see everything you've shipped |
| `"video/published"` | inside that workflow's own folder | you back up or move workflows individually, and want each one self-contained |
| `"/Volumes/Media/clips"` · `"~/Dropbox/Team/social"` | anywhere on disk | the files are large, or an editor, client or teammate needs them without access to your workspace |
...
Relative paths resolve from the workspace root; `~` and absolute paths work.

Two things to know before pointing it outside the workspace:

- **The `.gitignore` only covers `published/`.** A folder you create elsewhere
  inside the workspace needs its own ignore rule, or the first `git add` commits
  a video
...
Worth a scheduled task once the video workflow is running weekly —
